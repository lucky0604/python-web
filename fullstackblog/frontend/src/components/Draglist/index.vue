<template>
  <div class="twoDndList">
    <div class="towDndList-list" :style="{width: width1}">
      <h3>{{list1Title}}</h3>
      <draggable :list="list1" class="dragArea" :options="{group:'artilce'}">
        <div class="list-complete-item" v-for="element in list1">
          <div class="list-complete-item-handle">[{{element.user.username}}] {{element.title}}</div>
    ...
    <div class="twoDndList-list" :style="{width: width2}">
      ...
      <draggable :list="filterList2" class="dragArea" :options="{group:'article'}">
        <div class="list-complete-item" v-for="element in filterList2">
          <div class="list-complete-item-handle2" @click="pushEle(element)">[{{element.user.username}}] {{element.title}}</div>
        </div>
      </draggable>
    </div>
  </div>
