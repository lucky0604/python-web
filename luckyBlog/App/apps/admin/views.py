...
class ArticleAddView(LoginRequiredMixin, View):
  ...
  def post(self, request):
    ...
    url = d['url'][0]
    status = d['status'][0]
    editormd = d['editormd-markdown-toc'][0]
    try:
      tags = d['tags']
    except KeyError:
      tags = ''
...
class CategoriesView(LoginRequiredMixin, View):
  ...
  def post(self, request):
    ...
    for category in categories.split(','):
      try:
        Categories.objects.create(name = categories.capitalize())
      except Exception:
        pass
...
class ProfileView(LoginRequiredMixin, View):
  ...
  def post(self, request):
    ...
    if email and email != user.email:
      ...
      flag = True
    user.save()
    Message.objects().create(body = 'User %s changed files at %s' % (user.email, timezone.now()))
    if flag:
      logout(request)
